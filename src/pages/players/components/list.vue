<template>
  <up-dropdown>
    <up-dropdown-item v-model="showType" title="查看范围" active-color="#558dda"
      :options="playerCampOptions"></up-dropdown-item>
    <up-dropdown-item v-model="sortType" title="排序方式" active-color="#558dda"
      :options="playersOrderOptions"></up-dropdown-item>
  </up-dropdown>
  <scroll-view class="player-list-wrapper" refresher-enabled scroll-y :refresher-triggered="isTriggered"
    @refresherrefresh="onRefresherrefresh">
    <ListItem v-for="item in playerList.filter(p => showType === 'all' || showType === p.camp)" @tap="showPop(item)"
      :sortType="sortType" :datas="item" :key="item.id"></ListItem>
  </scroll-view>
  <up-popup v-model:show="isShowPop" v-if="popData" :customStyle="{ background: '#f4f8ff' }">
    <view class="player-detail-popup-content">
      ...
      <scroll-view class="pop-bd">
        ...
        <view v-for="opt in playersOrderOptions" :key="opt.name">
          <text class="title">{{ opt.name }}：</text>
          <text class="data-value">{{ popData?.seasonData && (['k', 'a', 'cost', 'mscs'].includes(opt.value) ?
            $formatnum(popData.seasonData[opt.value]) : $formatnum(popData.seasonData[opt.value], 0, true)) || '0'
            }}</text>
        </view>
        <view class="add-wrapper" v-if="userInfoStore?.userInfo?.roleId === 1 || userInfoStore?.userInfo?.roleId === 2"
          title="选手全部角色，用/分隔" type="line">
          <view class="add-text">添加角色：</view>
          <uni-easyinput v-model="newRoles" type="text" placeholder="" />
